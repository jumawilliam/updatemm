...
use Princetech\MobileMoney\Mpesa\Repositories\EndpointsRepository;
...
class Authenticator
{
    ...
    public function authenticate($bulk = false): ?string
    {
        ...
        if (config('Princetech.mpesa.cache_credentials', false) && !empty($key = $this->getFromCache())) {
            return $key;
        }
        try {
    ...
    {
        $key = \config('Princetech.mpesa.c2b.consumer_key');
        $secret = \config('Princetech.mpesa.c2b.consumer_secret');
        if ($this->alt) {
            //lazy way to switch to a different app in case of bulk
            $key = \config('Princetech.mpesa.b2c.consumer_key');
            $secret = \config('Princetech.mpesa.b2c.consumer_secret');
        }
